...
func run() error {
	...
	if err := cfg.Validate(); err != nil {
		logger.V(logging.ERROR).Error(err, "Invalid config. Processor cannot start", "err", err)
		return err
	}
	...
	if err != nil {
		logger.V(logging.ERROR).Error(err, "Failed to build processor clients")
		return err
	}

	if err := procClients.Validate(); err != nil {
		logger.V(logging.ERROR).Error(err, "Processor client validation failed")
		return err
	}
	...
	if cfg.TerminateOnObservabilityFailure {
		...
		if obsErr := waitObservabilityFatalError(obsFatalCh, 100*time.Millisecond); obsErr != nil {
			logger.V(logging.ERROR).Error(obsErr, "Processor stopped due to observability server failure")
			return obsErr
		}
	}
	if err != nil {
	...
	go func() {
		...
		reportFatal := func(err error) {
			...
			if !terminateOnObservabilityFailure {
				logger.V(logging.ERROR).Error(err, "Observability server failed in best-effort mode; processor will continue")
				return
			}
...
func waitObservabilityFatalError(obsFatalCh <-chan error, wait time.Duration) error {
