...
func TestBuildResult(t *testing.T) {
	...
	t.Run("HTTP error with JSON body", func(t *testing.T) {
		...
		if result.Response == nil {
			t.Fatal("expected response field for HTTP error")
		}
		...
		if errObj["message"] != "Invalid model" {
			t.Fatalf("expected message 'Invalid model', got %v", errObj["message"])
		}
		if errObj["code"] != "model_not_found" {
			t.Fatalf("expected code 'model_not_found', got %v", errObj["code"])
		}
	})
	...
	t.Run("DroppedReason other", func(t *testing.T) {
		clientErr := &inference.ClientError{
			Category:      httpclient.ErrCategoryRateLimit,
			Message:       "HTTP 429: Rate limit exceeded",
			StatusCode:    429,
		...
		result := buildResult(msg, nil, clientErr, logr.Discard())
		if result.Error != nil {
			t.Fatalf("expected nil error for non-TTL dropped reason, got %+v", result.Error)
		}
		if result.Response == nil {
			t.Fatal("expected response field for HTTP 429 with non-TTL dropped reason")
		}
