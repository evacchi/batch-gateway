...
func (p *Poller) FetchJobItem(ctx context.Context, task *db.BatchJobPriority) (*db.BatchItem, error) {
	...
	// re-enqueue might create a zombie job. delete the task from the queue.
	if len(jobs) == 0 {
		...
		if _, err := p.pq.PQDelete(ctx, task); err != nil {
			logger.V(logging.ERROR).Error(err, "Failed to delete the task from the queue")
		}
		return nil, nil
	}
