...
type dbGetErrWrapper struct {
	inner *mockdb.MockBatchDBClient
	err   error
}

func (d *dbGetErrWrapper) DBStore(ctx context.Context, job *db.BatchItem) (string, error) {
	return d.inner.DBStore(ctx, job)
}

func (d *dbGetErrWrapper) DBGet(ctx context.Context, query *db.BatchDBQuery, includeStatic bool, start, limit int) (
...
func TestPoller_FetchJobItem_DBError_ReEnqueuesAndReturnsError(t *testing.T) {
	...
	pq := &pqSpy{inner: mockdb.NewMockBatchPriorityQueueClient()}
	innerDB := mockdb.NewMockBatchDBClient()
...
func TestPoller_FetchJobItem_DBInconsistency_DeletesFromPQ_ReturnsNilNil(t *testing.T) {
	ctx := context.Background()
	...
	if pq.deleteCalled != 1 {
		t.Fatalf("PQDelete called=%d, want 1", pq.deleteCalled)
	}
}
...
func TestPoller_FetchJobItem_Found_ReturnsJobItem(t *testing.T) {
	...
	if _, err := dbClient.DBStore(ctx, seed); err != nil {
